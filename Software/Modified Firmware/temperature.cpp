...
static void temp_runaway_stop(bool isPreheat, bool isBed);
  ...
  float temp_ambient;
  float initial_temp_increase_PID; //added by Keisuke
    ...
    if(_millis() - temp_millis > 2000) {
      ...
      SERIAL_PROTOCOLLN(p);       
		if (safety_check_cycles == 0) { //save ambient temp
            ...
            // /* added by Keisuke
            initial_temp_increase_PID = 5.0;
            //SERIAL_PROTOCOLPGM("flag_temperature.cpp_initial_PID_temp_before");
            
            if (cs.extruder_type == 0) { // default 3D printer
                initial_temp_increase_PID = 5.0;
            } else if (cs.extruder_type == 1) { // wax printer
                initial_temp_increase_PID = 0.5;
                //SERIAL_PROTOCOLPGM("flag_temperature.cpp_initial_PID_temp_changed");
            }
            
            if (abs(input - temp_ambient) < initial_temp_increase_PID) {
...
//-//				if (_current_temperature - __preheat_start[_heater_id] < ((_isbed && (_current_temperature>105.0))?0.6:2.0)) {
                    __delta=2.0;
                    //SERIAL_PROTOCOLPGM("flag_temperature.cpp_delta_before");
                    if(_isbed)
                         {
                         __delta=3.0;
                    ...
                    // /* added by Keisuke
                    if (cs.extruder_type == 1) {
                        __delta = 0.2;
                        //SERIAL_PROTOCOLPGM("flag_temperature.cpp_delta_changed");
                    }
                    // */
