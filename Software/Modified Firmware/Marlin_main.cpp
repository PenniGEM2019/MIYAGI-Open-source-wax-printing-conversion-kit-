...
if((eSoundMode==e_SOUND_MODE_LOUD)||(eSoundMode==e_SOUND_MODE_ONCE))
	...
	break;
            ...
            // /* added by Keisuke
            //! ### M1001 - Define the type of extruder
            // ------------------------------------
            /*!
             
             M1001 [S]
             
             - 'S' define the type of extruder
             default value 0 is for 3D printing
             value 1 is for wax printing
             */
        case 1001:
        {
            if (code_seen('S')) {
                SERIAL_PROTOCOL(cs.extruder_type);
                SERIAL_PROTOCOLPGM("  ");
                cs.extruder_type = code_value();
                SERIAL_PROTOCOL(cs.extruder_type);
                SERIAL_PROTOCOLPGM("  ");
                if (cs.extruder_type == 0) {
                    SERIAL_PROTOCOLPGM("extruder type: 3D printer ");
                } else if (cs.extruder_type == 1) {
                    SERIAL_PROTOCOLPGM("extruder type: wax printer ");
                } else {
                    SERIAL_PROTOCOLPGM("extruder type: not recognized ");
                }
            } else {
                cs.extruder_type = 0;
                SERIAL_PROTOCOLPGM("extruder type: 3D printer ");
